	...
	componentDidMount(){
		...
		document.querySelectorAll(".shop-catagories").forEach( item => {
			...
			document.querySelectorAll(".brands-checkboxes").forEach( item => {
				...
				let selectedCatagory = item.getAttribute("data-catagory");
				console.log(selectedCatagory);
				
				fetch("./products.json")
				.then(response => response.json())
				.then(data => {
					let myArr = data.filter( (datum) => datum[selectedCatagory]==true);
		...
		document.querySelectorAll(".brands-checkboxes").forEach( item => 
			...
			let selectedCatagory = e.target.value;
				fetch("./products.json")
				.then(response => response.json())
				.then(data => {
					let myArr = data.filter( (datum) => datum['brand']==selectedCatagory);
					if(myArr.length > 6){ myArr = myArr.slice(0, 6)};
					console.log(myArr);
					console.log(myArr.length);
					let productsDiv = document.querySelector(".shop-content .row");
